...
```
builder ─ submitGasAuctionBid ──────► auction wins slot
          (register_token stream,            │
           if the asset is new)              ▼
builder ─ perpDeploy { RegisterAsset } ► AssetId allocated
          + lifecycle sub-variants          │
            (SetOracle, SetLeverage, ...)    ▼
          perpDeploy { ActivateMarket } ► market live, first block
