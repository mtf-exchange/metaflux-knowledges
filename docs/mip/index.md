...
The deployment proposals split spot from perp the same way established venues do: **MIP-1** is permissionless spot token + market deploy (the `spotDeploy` action family), **MIP-3** is permissionless builder-deployed perp markets (the `perpDeploy` action family). Both ride the same three gas-auction streams. (The current implementation still bundles both action families in one module and labels the spot path "MIP-3"; this is being realigned — behaviour unchanged.) **MIP-2 (Metaliquidity)** is the protocol-owned native-liquidity vault. **MIP-4** is the options product. Its first release is fully collateralized and cleared through [RFQ](../concepts/rfq.md) only; the margined options book is not built. **MIP-6** (Outcomes) was previously numbered MIP-4 and renumbered when MIP-4 was reassigned. **MIP-5 (Earn)** is the lending-pool supply side — depositors earn yield from the interest spot-margin borrowers pay, reusing the MIP-2 NAV/share model.

## V1 scope {#v1-scope}

V1 covers MIP-1, MIP-2, MIP-3, and the collateralized first release of MIP-4. The margined MIP-4 book is targeted for V2; MIP-6 (Outcomes) is deferred to V3. MIP-5 (Earn) is live: deposit and redeem work, and it pays zero until two governance votes land — see [Earn](../concepts/earn.md).
