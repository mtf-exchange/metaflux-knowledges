...
**Gating.** Rejected if the account is a split `standard` account (`spot margin is not available in standard mode` — not live until the node 0.9.7 swap, see [the standard-mode split](../../../concepts/usdc.md#standard-split)), if margin is not enabled for the pair, if a position is already open on the pair, if your free collateral is below the initial-margin requirement, if the Earn pool's idle liquidity is below `borrow`, if spot trading is halted, or on a zero `size` / non-positive `borrow`.
