...
| **A read left the public API** — `protocol_metrics`, `mip3_deployer_oracle`, `fba_batch_state` | Operator lane. Every public fact `protocol_metrics` carried is on `markets`, `markets_meta` and `staking_state`; the FBA read ships publicly with its engine |
