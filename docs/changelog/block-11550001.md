---
description: What changes on chain 114514 at block 11,550,001 — vault_modify signs a new type, every raw-size row states its own size plane, one faucet claim per address, a parked per-order status, a replayed-nonce verdict, per-leg cloid dedup, three silent accepts that become refusals, and six rules with no height pin. The gateway rows that shipped the same day are at the end.
---
...
several. The rows below are the ones a caller can observe. The release also
carries [six rules with no height pin](#no-height-pin). Anything else in the
...
## Rules with no height pin {#no-height-pin}

The rules below have no height pin. They turned on when node 0.9.10 took over,
so they also apply from block 11,550,001.

| Rule | Before the swap |
|---|---|
| A [spot order](../api/rest/exchange/spot.md) that the spot balance cannot fund at all is refused: `ASSET_INSUFFICIENT_BALANCE`, `insufficient spot balance`. A spot [`scale_order`](../api/rest/exchange/orders.md#scale_order) rung and a spot [`chase_order`](../api/rest/exchange/orders.md#chase_order) leg get the same refusal | Accepted as a no-op that answered `filled` with `total_sz: "0"`. An unfunded chase leg answered `chase leg did not rest` |
| A split `standard` account has no reservations. Its perp and option orders are admitted against the perp wallet with no cap. `available_to_trade` has no cap, `account_state` omits `reservations`, and [`user_set_abstraction`](../api/rest/exchange/account.md#user_set_abstraction) kinds 1–3 are refused with `a split standard account has no reservations` | The `perp` and `option` reservations capped the account, so a split account with no `perp` reservation opened no perp position |
| [`usd_class_transfer`](../api/rest/exchange/transfers.md#usd_class_transfer) from a short spot wallet answers `insufficient spot balance`, with `ASSET_INSUFFICIENT_BALANCE` | `insufficient spot USDC balance` |
| A [perp delist](../products/perpetuals.md#delisting) closes every open position and marks the market settled. `markets` gains `settled` and `settled_px`, orders get `market settled — trading closed`, and `ledger_updates` gains `cause: "delist_settlement"` | A delist halted the market and left its positions open |
| [Funding](../concepts/funding-rates.md#paused-market) stops while closing is disabled or the market is settled | Funding settled on every market, whatever its flags |
| A [`staking_state`](../api/rest/info/vaults-staking.md#staking_state) delegation row carries `lock_months` and `reward_weight` | The row carried neither key |
