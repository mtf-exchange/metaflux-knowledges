...
| [Next release and unverified wire rows](./next-release.md) | **Not live yet:** a 1 MiB cap on the signed `action` bytes. Two rows are in the shipped code but not yet read on the running chain: the rejected-leg `error` level and the size plane on archive candles. Also two corrections to this reference. |
