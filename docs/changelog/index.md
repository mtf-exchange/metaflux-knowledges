...
| [Unverified wire rows](./next-release.md) | Nothing here waits for a node release. Two rows are in the shipped code but not yet read on the running chain: the rejected-leg `error` level and the size plane on archive candles. Also two corrections to this reference. |
