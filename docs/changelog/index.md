...
| [Next release and unverified wire rows](./next-release.md) | **Not live yet:** a 1 MiB cap on the signed `action` bytes, and a `batch_cancel` reply and `order_updates` record for each leg. Two rows are in the shipped code but not yet read on the running chain: the rejected-leg `error` level and the size plane on archive candles. Also two corrections to this reference. |
...
| [Block 11,550,001](./block-11550001.md) | `vault_modify` signs a new type, every raw-size row states its size plane, one faucet claim per address, a `parked` status, a `NONCE_REPLAYED` verdict, per-leg `cloid` dedup, three silent accepts that become refusals. Also the gateway rows that shipped the same day: four reads that answer `410`, one snapshot frame per subscribe, a `trades` limit that caps the merged answer. |
