...
across the EVM contract and the L1 (cross-language known-answer vectors). A
pre-mainnet audit remains before mainnet.
:::
...
- **Replay** — each `message_id` is honored once, keyed on the chain/source-nonce
  ...
  rotation; enforced on both the L1 and the contract (`withdrawalSeen`).
  Signatures are domain- and epoch-bound, so a cosignature
...
- **Governance & rotation** — no admin account; every privileged op is
  ...
  so a compromised governance quorum cannot silently swap the set.
...
All co-signed calls take `(uint8[] sigV, bytes32[] sigR, bytes32[] sigS)` ordered by
ascending signer, low-S, `v ∈ {27,28}`. The contract rejects high-S signatures and
binds the contract id + epoch into every co-signed digest.
...
- Multi-chain rollout: Arbitrum Sepolia is live under the same model, alongside
  Base Sepolia.
