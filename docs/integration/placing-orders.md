...
| `{"noop":{"reason":"…"}}` | Accepted, and it changed nothing — a `reduce_only` leg with nothing left to reduce. **A success. Do not retry it.** No `oid`. See [`noop`](../api/rest/exchange.md#statuses-noop) |
