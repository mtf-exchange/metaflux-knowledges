...
master                                          chain
  │                                              │
  ├── ApproveAgent { agent, expires_at_ms }────► │  approval recorded
  │                                              │   under master's account
  │                                              │
  │       ┌────── agent key signs ───────┐       │
  ├──────►│ Order / Cancel / Transfer    │──────►│  /exchange admits because
  │       │ sender = master              │       │   recovered signer is an
  │       │ signature = sign_agent(...)  │       │   active agent of sender
  │       └──────────────────────────────┘       │
  │                                              │
```
