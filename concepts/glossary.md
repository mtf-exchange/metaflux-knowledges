...
**Backstop (T3).** Liquidation tier where the protocol seizes a sub-threshold account's position into the insurance pool. See [tiered liquidation](./tiered-liquidation.md#t3-backstop).
...
**Builder rebate.** Fee share paid to the address that originated an order (front-end, aggregator, automation service). See [fees](./fees.md#builder-rebate).
...
**Funding rate.** Hourly user-to-user payment that pegs perp price to underlying oracle. See [funding rates](./funding-rates.md).
...
**Oracle.** External price feed composed from CEX prices via TWA. Input to mark price + funding. See [mark prices](./mark-prices.md#oracle-composition).
