...
deployer                                 chain
  │                                       │
  │ VaultDeploy { name, strategy_desc,    │
  │   performance_fee_bps,                │
  │   management_fee_bps,                 │
  │   lock_period_ms,                     │
  │   manager: 0x<mgr_addr> }             │
  ...
  │  ──────────────►                      │
  │                                       │
  │ manager trades (signs as vault_addr)  │
  │  Order { ... } / Cancel / etc.        │
  │  ──────────────►                      │
  │                                       │
  │ vault P&L updates share_price         │
  │                                       │
  │ users withdraw                        │
  │  VaultWithdraw { vault, shares }      │
  │  ──────────────►                      │
```
...
A subset of MFlux Vault is the **insurance pool** — a designated reserve that draws down during T3 backstop events. See [tiered liquidation](./tiered-liquidation.md#t3-backstop).
...
```
T=0    user A deposits 1000 USDC
       vault NAV: 0 + 1000 = 1000
       shares_outstanding: 0 + 1000 = 1000 (1e8 share-price)

T+1    user B deposits 1000 USDC
       vault NAV: 1000 + 1000 = 2000
       shares_outstanding: 1000 + 1000 = 2000

T+2    manager opens a 2 BTC long at mark 100
       vault NAV: 2000 (unrealised 0)

T+3    mark rises to 110
       unrealised PnL on long: +20
       vault NAV: 2020; share_price: 1.01

T+4    perf fee triggers on the +0.01 share-price gain
       fee = 0.01 × 2000 × 0.15 = 3
       NAV after fee: 2017
       share_price after: 1.0085

T+5    user A withdraws all (1000 shares)
       payout: 1000 * 1.0085 = 1008.5 USDC
       NAV: 2017 - 1008.5 = 1008.5
       shares_outstanding: 1000
       share_price: still 1.0085
```
