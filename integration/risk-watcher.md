...
│   shares signing key with risk-watcher (or uses sep) │
└──────────────────────────────────────────────────────┘
                │
                │ runs in same process or sidecar
                ▼
┌──────────────────────────────────────────────────────┐
│ risk-watcher                                         │
│   WS subscribe: marginEvents { user }                │
│   on tier ≥ T0: take action                          │
│   on health < 1.2: pre-emptive top-up                │
│   on tier T1+: emergency unwind                      │
└──────────────────────────────────────────────────────┘
                │
                │ submits agent-signed actions
                ▼
            POST /exchange
```
...
```
T = 0       health = 1.6  (Safe)
T = 1s      mark drops 1%; health = 1.4  → sample drop
T = 2s      mark drops 0.5%; health = 1.3 → 2nd drop
T = 3s      ...                            → 3rd
T = 4s      ...                            → 4th
T = 5s      health = 1.0  → 5 samples falling; pre-empt
T = 5s      compute needed = (1.8 - 1.0) × maint = 0.8 × maint
T = 5.1s    submit UpdateIsolatedMargin deposit
T = 5.2s    202 admitted
T = 5.3s    commit; health = 1.8 → Safe
T = 5.3s    marginEvents push: tier=Safe; reaction loop continues
```
