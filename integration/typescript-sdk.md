...
| `baseUrl` | string | yes | Gateway front door (`https://gateway.<net>.mtf.exchange`). The SDK speaks MTF-native, which is the gateway's default path (`/info` · `/exchange` · `/ws`); HL-compat lives under `/hl/*`. Running the node yourself? Point at `http://localhost:8080`. See [networks](../networks.md). |
