...
| المسار | شكل الإرسال |
|------|-----------|
| `POST /info` (البوابة الافتراضية) | MTF-native (هذه الوثيقة) |
| `POST /hl/info` (البوابة، تحت `/hl`) | **HL-compat** — انظر [hl-compat.md](./hl-compat.md) |

MTF-native هو المسار الافتراضي للبوابة؛ وHL-compat مُحدَّد بالفضاء الاسمي تحت `/hl/*`.
عند تشغيل العقدة بنفسك، يُقدَّم نفس المسار الأصلي `/info` مباشرةً على
`http://localhost:8080`.
...
## جدول التكافؤ: اسم مستعار hl-compat ← النوع الأصلي MTF

تعرض واجهة [hl-compat](./hl-compat.md) للبوابة كل نوع لقطة عقدة تحت اسم مستعار camelCase للعملاء المتوافقين مباشرةً. العمود الأيسر هو ذلك الاسم المستعار؛ الوسطى هو نوع عقدة MTF الأصلي الذي يُعيَّن إليه؛ ✅ = مخدوم، ⚠️ = مخدوم مع وكيل مُعلَّم (لا يوجد دعم مباشر في الحالة الملتزمة).

| اسم مستعار hl-compat | النوع الأصلي MTF | الحالة | ملاحظات |
|----------------------------|------------------------------|--------|-------|
| `meta` | `markets` | ✅ | مخدوم بالفعل |
| `spotMeta` | `spot_meta` | ✅ | `mip3_spot_pair_specs` + `mip3_spot_token_specs` |
| `clearinghouseState` | `account_state` | ✅ | مخدوم بالفعل |
| `spotClearinghouseState` | `spot_clearinghouse_state` | ✅ | مُفهرَس بـ `(owner, asset)` |
| `exchangeStatus` | `exchange_status` | ✅ | أعلام قيمة واحدة |
| `openOrders` | `open_orders` | ✅ | مخدوم بالفعل |
| `frontendOpenOrders` | `frontend_open_orders` | ✅ | + trigger / tif / cloid |
| `liquidatable` | `liquidatable` | ✅ | مؤشر BOLE (⚠️ فارغ حتى أول تمرير BOLE) |
| `activeAssetData` | `active_asset_data` | ✅ | مركز / إعدادات / سوق |
| `maxMarketOrderNtls` | `max_market_order_ntls` | ⚠️ | سقف OI كوكيل لحد الحجم |
| `vaultSummaries` | `vault_summaries` | ✅ | ⚠️ `tvl` = وكيل NAV بأعلى علامة مائية |
| `userVaultEquities` | `user_vault_equities` | ✅ | مُفهرَس لكل خزنة |
| `leadingVaults` | `leading_vaults` | ✅ | مُصفَّى حسب القائد |
| `userRateLimit` | `user_rate_limit` | ✅ | `UserActionStats` |
| `spotDeployState` | `spot_deploy_state` | ✅ | مزاد غاز spot |
| `delegatorSummary` | `delegator_summary` | ✅ | ملخص الرهن التجميعي |
| `maxBuilderFee` | `max_builder_fee` | ✅ | `approved_builders` |
| `userToMultiSigSigners` | `user_to_multi_sig_signers` | ✅ | `MultiSigConfig` |
| `userRole` | `user_role` | ✅ | مستنتج من السجلات |
| `perpsAtOpenInterestCap` | `perps_at_open_interest_cap` | ✅ | OI مقابل `oi_cap` |
| `validatorL1Votes` | `validator_l1_votes` | ✅ | بيانات وصفية (حمولة غير شفافة) |
| `validatorSummaries` | `validator_summaries` | ✅ | الرهن / العمولة / أعلام النشاط والسجن |
| `gossipRootIps` | `gossip_root_ips` | ✅ | نقاط نهاية النظراء المُهيَّأة (فارغة في العقدة المنفردة) |
| `marginTable` | `margin_table` | ⚠️ | مستوى فعّال واحد لكل سوق (لا سلّم متعدد الصفوف) |
| `perpDexs` | `perp_dexs` | ✅ | مؤشر + عدد الأصول |
| `webData2` | `web_data2` | ✅ | مُجمَّع |
| `userFees` | `fee_schedule` | ✅ | مخدوم بالفعل |
| `delegations` | `staking_state` | ✅ | مخدوم بالفعل |
| `perpDeployAuctionStatus` | `mip3_active_bids` | ✅ | مخدوم بالفعل |
| `subAccounts` | `sub_accounts` | ✅ | مخدوم بالفعل |
