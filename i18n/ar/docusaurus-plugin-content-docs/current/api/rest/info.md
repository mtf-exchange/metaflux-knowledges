...
## أنواع الاستعلام {#query-types}

### هوية العقدة الثابتة وإصدار البروتوكول {#node_info}
...
### الهامش والمراكز والأرصدة لكل حساب {#account_state}
...
### ملخص هامش خفيف لكل حساب {#margin_summary}
...
### القيمة الإجمالية المُدارة وسعر الحصة والاستراتيجية لكل خزينة {#vault_state}
...
### حالة التخزين والتفويض لكل حساب {#staking_state}
...
### رسوم الصانع والمتلقي المتدرجة حسب الحجم {#fee_schedule}
...
أسعار الرسوم هي **نقاط أساس** عشرية كنصوص برقم عشري واحد (مثل: `"2.0"` = 2 نقطة أساس = 0.02%، و`"0.5"` = 0.5 نقطة أساس = 0.005%)، مما يتيح دقة أدق من نقطة الأساس. `burn_ratio` هو كسر عشري (`"0.30"` = 30% من الرسوم تُحرَق). انظر [الرسوم](../../concepts/fees.md).

### أوامر الحساب المعلَّقة عبر جميع دفاتر العقود الدائمة {#open_orders}
...
يُحدَّد الحساب بواسطة `address` (0x hex). غياب `address` ←
...
### سجل التنفيذات الأخيرة لحساب {#user_fills}
...
|-----|------|----------|-------------|
| `address` | hex address | نعم | عنوان الحساب |
...
|-------|------|-------------|
| `address` | hex address | عنوان الحساب المُحلَّل |
| `fills[*].coin` | string | رمز السوق الذي نُفِّذ عليه التنفيذ |
...
### سجل التنفيذات مُصفًّى وفق نافذة زمنية {#user_fills_by_time}
...
### البحث عن دورة حياة أمر منفرد {#order_status}
...
### بيانات تعريفية لآخر كتلة مُلتزَم بها {#block_info}
...
### محافظ الوكلاء المعتمدة لحساب {#agents}
...
غياب `address` ← `400 {"error":"missing field address"}`.
...
### قائمة الحسابات الفرعية لحساب {#sub_accounts}
...
### عدادات ومُجمِّعات على مستوى البروتوكول {#protocol_metrics}
...
### مستوى الرسوم والحجم لكل حساب {#user_fees}
...
### معدل العائد السنوي الفعلي للتخزين ومدخلاته {#staking_apr}
...
### مجموعة مصادر الأوراكل الفرعية لكل سوق {#oracle_sources}

مجموعة مصدر الأوراكل الفرعية المُثبَّتة لكل سوق. يُحلَّل السوق بواسطة `coin` (الرمز).
...
غياب `coin` ← `400 {"error":"missing field coin"}`؛ سوق غير معروف ←
...
| الحقل | النوع | الوصف |
|-------|------|-------------|
| `coin` | string | رمز السوق المُردَّد / المُحلَّل |
...
## أنواع استعلامات الحوكمة {#governance-query-types}
...
### حالة الحوكمة الحية والمعاملات الحالية {#gov_state}

سطح الحوكمة الحي — سياق نصاب الحصة، جولات تصويت معلَّقة لتغيير المعاملات،
مقترحات مفتوحة، والقيمة الحالية لكل معامل محكوم.
لا يتطلب أي معاملات.
...
| `open_proposals[*].proposal_id` | uint64 | معرّف جولة المقترح |
...
### مقترحات الحوكمة النشطة وحالة النصاب {#gov_proposals}

كل مقترح حوكمة **نشط** عبر **جميع** فئات التصويت (وليس تصويتات تغيير المعامل
المباشرة فقط)، مع إحصاء الحصة الحية لكل حمولة والمسافة إلى نصاب ⅔.
...
| `proposals[*].proposal` | object \| null | سجل المقترح المكتوب عندما تُفتح الجولة عبر مقترح، وإلا `null` |
...
### سجل تغييرات معاملات الحوكمة المُفعَّلة {#gov_history}
...
| `enacted[*].via` | `"proposal" \| "vote_global" \| "other"` | مسار المصدر — مُتتبَّع عبر مقترح مقابل تصويت مباشر لمعامل |
...
## أنواع الاستعلامات المتقدمة (RFQ / FBA / هامش المحفظة) {#advanced-query-types-rfq--fba--portfolio-margin}
...
### طلبات RFQ المفتوحة وعروض أسعار صانعي السوق {#rfq_open}
...
### طلبات RFQ التي طلبها الحساب أو قدَّم عروضاً عليها {#rfq_user}
...
يُحدَّد الحساب بواسطة `address` (0x hex). غياب `address` ←
`400 {"error":"missing field address"}`؛ `address` غير صالحة ← `400 {"error":"invalid hex"}`.
...
### حوض FBA الحي والتسوية الاسترشادية {#fba_batch_state}
...
غياب `coin` ← `400 {"error":"missing field coin"}`. لا يُعاد **404** لسوق غير مسجّل: FBA يعتمد على الاشتراك بالسوق، لذا يُعيد سوق بلا حوض **200** مع حقول قيمتها صفر (`enabled:false`، `period_ms:0`، `orders` فارغة، `indicative:null`).
...
| الحقل | النوع | الوصف |
|-------|------|-------------|
| `coin` | string | رمز السوق المُعاد إرساله |
...
### تسجيل هامش المحفظة وأرقام السيناريو {#pm_summary}
...
يُحدَّد الحساب بواسطة `address` (0x hex). غياب `address` ←
`400 {"error":"missing field address"}`. يُعيد الحساب غير المسجَّل **200** مع `enrolled:false` وأرقام قيمتها صفر.
...
## أنواع استعلامات لقطة العقدة {#node-snapshot-query-types}
...
## أنواع استعلامات لقطة العقدة العامة {#general-node-snapshot-query-types}

قراءات لقطة العقدة غير المرتبطة بمنتج تداول واحد — حالة البورصة، مساعدات الواجهة الأمامية / الأوامر المفتوحة، التصفية، حدود المعدل، الخزائن، المحقّقون، والتوقيع المتعدد.

### حالة التداول العالمية للبورصة {#exchange_status}
...
### الأوامر المفتوحة مع تفاصيل مدة الصلاحية والتفعيل {#frontend_open_orders}
...
### ملخص جميع الخزائن {#vault_summaries}
...
### الخزائن التي أودع فيها المستخدم {#user_vault_equities}
...
### الخزائن التي يقودها المستخدم {#leading_vaults}
...
### إحصاءات إجراءات المستخدم وميزانية حدود المعدل {#user_rate_limit}
...
### ملخص التخزين لعنوان {#delegator_summary}
...
### سقف رسوم المُنشئ المعتمَد {#max_builder_fee}

سقف رسوم المُنشئ المعتمَد للزوج `(address, builder)`. مطلوب: `address` (0x hex) + `builder` (0x hex).
...
### إعدادات التوقيع المتعدد لعنوان {#user_to_multi_sig_signers}
...
### الدور المُستنتَج للحساب {#user_role}
...
### بيانات تصويت الأوراكل الحالية لكل مُحقِّق {#validator_l1_votes}
...
### لقطة الحصة والحالة لكل مُحقِّق {#validator_summaries}
...
### نقاط نهاية النظراء الجذريين/البذريين المُهيَّأة للنشر الإذاعي {#gossip_root_ips}
...
### `web_data2` — أُزيل {#web_data2--removed}

:::warning
**تمت إزالة `web_data2`** (سواء نوع REST الخاص بـ `/info` أو قناة WS).
يُعيد الطلب الآن `400 {"error":"unknown info type: web_data2"}`؛ ويُعيد اشتراك
WS `{"channel":"error","data":{"error":"unknown channel: web_data2"}}`.

استخدم بدلاً منه القراءات المُركَّزة المكافئة — فهي تحمل البيانات ذاتها بأشكال
ثابتة ومُصدَرة بشكل مستقل:

| قسم `web_data2` القديم | استخدم بدلاً منه |
|-------------------------|-------------|
| `clearinghouse` (الهامش + المراكز) | [`account_state`](#account_state) (REST) / قناة WS الخاصة بـ `account_state` |
| `spot_balances` | [`spot_clearinghouse_state`](./info/spot.md#spot_clearinghouse_state) (REST) / قناة WS الخاصة بـ `spot_state` |
...
| 400 | `{"error":"missing field: address"}` / `{"error":"missing field coin"}` | وسيط خاص بالنوع مطلوب وغائب (يتفاوت التحجيم حسب القارئ) |
| 400 | `{"error":"invalid hex"}` | وسيط العنوان مشوَّه |
| 404 | `{"error":"market not found"}` | رمز `coin` مجهول (`market_info` وغيره) |
...
## تسلسل — الاستعلام عن حساب ورؤية أوامرك الخاصة {#sequence--query-an-account-see-your-own-order}
...
**س: كيف أُحدِّد السوق — بالمعرّف أم بالاسم؟**
ج: برمز `coin` (مثل `"BTC"`). أُزيلت وسائط الطلب الرقمية القديمة `asset_id` / `market_id`؛ يُقبل `coin` فقط، وتعرض الاستجابات رموز العملات في كل مكان. (لا يزال مسار الكتابة الموقَّع `/exchange` يستخدم `asset` الرقمي — هذا الحقل مُجمَّد بموجب الإجماع ولا علاقة له بوسائط القراءة هذه.)
