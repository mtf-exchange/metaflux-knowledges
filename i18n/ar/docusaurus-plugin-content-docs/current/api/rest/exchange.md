...
| [`set_metaliquidity_whitelist`](#set_metaliquidity_whitelist) | تصويت القائمة البيضاء لـ MLP | مفتاح المحقق |
| [`register_metaliquidity_operator`](#register_metaliquidity_operator) | تسجيل / إلغاء تسجيل مشغّل استراتيجية | قائد الخزينة |
...
| `AppHashVote` | — | إجراء محقق/نظام؛ يسير عبر مسار الإجماع، ولا يمر بـ `/exchange` أبدًا |
...
**الاستجابة.** تُعيد مغلف القبول [`202 Accepted`](#202-accepted--non-order-admission) (وليس `oid` متزامناً). تأكد من رصيد الضمان المُضاف عبر [`/info` `spot_margin_state`](./info.md#spot_margin_state). راجع [هامش السبوت](../../products/spot-margin.md).
...
**الاستجابة.** تُعيد مغلف القبول [`202 Accepted`](#202-accepted--non-order-admission). تأكد عبر [`/info` `spot_margin_state`](./info.md#spot_margin_state).
...
**الاستجابة.** تُعيد مغلف القبول [`202 Accepted`](#202-accepted--non-order-admission) (وليس `oid` متزامناً — تعبئة الأمر IOC الداخلي تأثير مُلتزَم به). راقب الـ`borrowed` / الـ`base_held` الناتجَين عبر [`/info` `spot_margin_state`](./info.md#spot_margin_state)؛ يتغير `total_borrowed` للمجمع في [`earn_state`](./info.md#earn_state). راجع [هامش السبوت](../../products/spot-margin.md).
...
**الاستجابة.** تُعيد مغلف القبول [`202 Accepted`](#202-accepted--non-order-admission). تأكد من الإغلاق الكامل مقابل الجزئي والمبلغ المُسدَّد عبر [`/info` `spot_margin_state`](./info.md#spot_margin_state) (الحساب المحذوف لن يظهر بعد الآن)؛ تأثيرات جانب المورِّد تظهر في [`earn_state`](./info.md#earn_state).
...
**الاستجابة.** تُعيد مغلف القبول [`202 Accepted`](#202-accepted--non-order-admission). تأكد من الحصص المُكتسبة / حصتك عبر [`/info` `earn_state`](./info.md#earn_state) (مرِّر `user` لتضمين `user_shares` / `user_value` الخاصة بك). راجع [Earn](../../concepts/earn.md).
...
**الاستجابة.** تُعيد مغلف القبول [`202 Accepted`](#202-accepted--non-order-admission)؛ قد يكون عدد الحصص المحروقة **أقل من المطلوب** عند تحديد الدفع بالخامل. تأكد من الحصة المتبقية وإجماليات المجمع عبر [`/info` `earn_state`](./info.md#earn_state). راجع [Earn](../../concepts/earn.md).
...
| `AppHashVote` | — | إجراء مُدقِّق/نظام؛ مسار التوافق فقط | — |
