...
## اصطلاحات الأرقام {#numeric-conventions}
...
## دلالات الموقِّع {#signed-by-semantics}
...
## كتالوج الإجراءات {#action-catalog}
...
### وضع الأوامر ودورة حياتها {#order-placement--lifecycle}
...
### هامش Spot وEarn {#spot-margin--earn}
...
### التخزين والتجريد {#staking--abstraction}
...
### الأوامر المشفَّرة {#encrypted-orders}
...
### سحوبات الجسر {#bridge-withdrawals}
...
### الإجراءات غير المتاحة على المسار العام `/exchange` {#not-on-the-public-exchange-path}

تظهر أسماء الإجراءات هذه في المسودات السابقة، لكنها **غير مُجسَّرة على معالج `/exchange` الأصلي لـ MTF**. إما أنها كتابات امتيازية / نظامية لا يجب أن تعبر مسار المستخدم العام، أو هي نماذج مخطط معترف بها لكن غير مُعيَّنة. إرسالها يُعيد `400 unsupported action`. انظر [الجدول أدناه](#non-bridged-actions) لمعرفة موضع كل منها.
...
| (دورة حياة PM) | `pm_enroll` / `pm_unenroll` | يُطابق [`user_portfolio_margin`](#user_portfolio_margin) (تسجيل / إلغاء تسجيل). تمت **إزالة** `pm_rebalance` — يُرفض بوصفه إجراءً غير معروف |
...
## إجراءات أوامر العقود الدائمة {#perpetual-order-actions}
...
#### `position_side` (وضع التحوط) {#position_side-hedge-mode}
...
#### أوامر التشغيل (`stop_loss` / `take_profit`) {#trigger-orders-stop_loss--take_profit}
...
### وضع عدة أوامر بتوقيع واحد {#batch_order}
...
### إلغاء أمر واحد بواسطة المعرّف {#cancel_order}
...
### إلغاء عدة أوامر بتوقيع واحد {#batch_cancel}
...
### إلغاء أمر بواسطة معرّف العميل {#cancel_by_cloid}
...
### إلغاء جميع الأوامر المعلّقة {#cancel_all_orders}
...
### تعديل سعر أو حجم أمر معلّق {#modify}
...
### تعديل عدة أوامر بتوقيع واحد {#batch_modify}
...
### جدولة مشغّل إلغاء شامل مستقبلي {#schedule_cancel}
...
### جدولة أمر TWAP مُقسَّم {#twap_order}
...
### إلغاء أمر TWAP قيد التشغيل {#twap_cancel}
...
## إجراءات تداول السبوت {#spot-trading-actions}
...
### إلغاء أمر سبوت معلّق {#spot_cancel}
...
## إجراءات هامش السبوت والعائد {#spot-margin--earn-actions}
...
### إيداع ضمانة لهامش السبوت {#spot_margin_deposit}
...
### سحب ضمانة هامش السبوت الحرة {#spot_margin_withdraw}
...
### فتح مركز سبوت برافعة مالية {#spot_margin_open}
...
### إغلاق مركز سبوت برافعة مالية {#spot_margin_close}
...
### توريد عرض إلى مجمع Earn {#earn_deposit}
...
### استرداد حصص مجمع Earn {#earn_withdraw}
...
## إجراءات هامش العقود الدائمة والمخاطر {#perpetual-margin--risk-actions}
...
### ضبط الرافعة المالية ووضع الهامش {#update_leverage}
...
### تعديل الهامش المعزول بدلتا {#update_isolated_margin}
...
### إضافة هامش إلى مركز معزول صارم {#top_up_isolated_only_margin}
...
### تسجيل أو إلغاء تسجيل هامش المحفظة {#user_portfolio_margin}
...
## إجراءات الحساب والتخزين والخزائن والجسر {#account-staking-vaults--bridge-actions}
...
### اعتماد محفظة وكيل {#approve_agent}
...
### تعيين اسم الحساب المعروض {#set_display_name}
...
### الربط بعنوان مُحيل {#set_referrer}
...
### اعتماد سقف رسوم البنّاء {#approve_builder_fee}
...
### تحويل الحساب إلى توقيع متعدد {#convert_to_multi_sig_user}
...
### إنشاء حساب فرعي {#create_sub_account}
...
### تحويل الضمانة بين الحساب الرئيسي والفرعي {#sub_account_transfer}
...
### تحويل رموز سبوت بين الحساب الرئيسي والفرعي {#sub_account_spot_transfer}
...
### تبديل وضع المركز الأحادي/المزدوج {#set_position_mode}
...
### نقل MTF إلى رصيد التخزين الحر {#c_deposit}
...
### نقل MTF خارج رصيد التخزين {#c_withdraw}
...
### تفويض أو إلغاء تفويض الحصة {#token_delegate}
...
### المطالبة بمكافآت التخزين {#claim_rewards}
...
### تعيين اسم مستعار لهدف التخزين {#link_staking_user}
...
### تبديل علامة تجريد DEX للحساب {#user_dex_abstraction}
...
### ضبط إعدادات التجريد ذاتية النطاق {#user_set_abstraction}
...
### ضبط إعدادات تجريد مستخدم آخر {#agent_set_abstraction}
...
### دفع رسوم أولوية للتقديم الأمامي في الكتلة {#priority_bid}
...
### إرسال أمر مشفَّر بعتبة {#submit_encrypted_order}
...
### تحويل الأموال بين القائد والخزينة {#vault_transfer}
...
### تحديث إعدادات الخزينة {#vault_modify}
...
### استرداد حصص الخزينة {#vault_withdraw}
...
### تحويل USDC من Core إلى EVM {#core_evm_transfer}
...
### سحب USDC إلى سلسلة خارجية {#mb_withdraw}
...
### الإجراءات غير المجسَّرة {#non-bridged-actions}
...
| (دورة حياة PM) | `pm_enroll` / `pm_unenroll` | يُطابق مع الإجراء القياسي للتسجيل/إلغاء التسجيل؛ تمت إزالة `pm_rebalance` ← يُرفض بوصفه إجراءً غير معروف | [`user_portfolio_margin`](#user_portfolio_margin) |
...
### `200 OK` — مسار الأوامر (oid متزامن) {#200-ok--order-path-synchronous-oid}
...
### `202 Accepted` — قبول إجراء غير خاص بالأوامر {#202-accepted--non-order-admission}
...
### `400 Bad Request` — طلب مشوَّه {#400-bad-request--malformed}
...
### `401 Unauthorized` — فشل التوقيع / التخويل {#401-unauthorized--signature--authorization-failed}
...
### `429 Too Many Requests` — تجاوز حد الطلبات {#429-too-many-requests--rate-limited}
...
### `503 Service Unavailable` — mempool ممتلئ {#503-service-unavailable--mempool-full}
...
## القبول ≠ الإيداع {#admission--commit}
...
## مخطط التسلسل — تقديم أمر ورؤيته في دفتر الأوامر {#sequence-diagram--place-an-order-and-see-it-on-the-book}
