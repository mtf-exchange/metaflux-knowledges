...
- **2xx** — نجاح. تستخدم نقاط النهاية الأصلية لـ MTF رموز حالة HTTP الصحيحة للأخطاء، لا أعلامًا داخل جسم الاستجابة.
...
### 401 — غير مُصرَّح له (فشل التوقيع) {#401--unauthorized-signature-failed}
...
### 404 — غير موجود {#404--not-found}

| `error` | يُثار عند |
|---------|----------------|
| `account not found` | `/info` استُعلِم بعنوان لا توجد له حالة على السلسلة |
| `market not found` | رمز `coin` غير مسجَّل في السجل |
| `vault not found` | `vault_id` غير موجود |
| `order not found` | `Cancel` لمعرّف طلب تم إلغاؤه بالفعل / تنفيذه / لم يوجد أصلًا |

بالنسبة لاستعلامات `/info`، ترجع نقاط النهاية الأصلية لـ MTF القيمة `404` عندما يكون المورد المُستعلَم عنه غير معروف.

### 405 — الطريقة غير مسموح بها {#405--method-not-allowed}

| `error` | يُثار عند |
|---------|----------------|
| (لا جسم) | استُخدم `GET` على نقطة نهاية `POST` (أو العكس) |

### 422 — كيان غير قابل للمعالجة {#422--unprocessable-entity}
...
### 429 — تجاوز حد المعدل {#429--rate-limited}
...
### 503 — الخدمة غير متاحة {#503--service-unavailable}
...
### أخطاء وقت الالتزام (ليست HTTP، في تدفق الأحداث) {#commit-time-errors-not-http-in-event-stream}

بعض الإخفاقات تحدث بعد `202 Accepted` لأنها لا تكون معروفة إلا في سياق تنفيذ الكتلة. تظهر هذه الأخطاء على قناة الـ WS الخاصة بـ `order_updates` / `user_events` على النحو التالي: `{"error":"<reason>", "action_hash":"0x..."}`.
