...
| الميزانية | الحد (الافتراضي) | إعادة التعبئة | الانطلاقة القصوى | الإعفاء |
|-----------|-----------------|---------------|-----------------|---------|
| وزن لكل IP | 1200 وزن / دقيقة | 20 وزن / ثانية | 1200 (دلو كامل) | عناوين IP المُدرجة في القائمة البيضاء معفاة |
| دلو `/exchange` لكل حساب | 30 طلب / ثانية | 30 / ث | 60 | الموقّعون ضمن مجموعة metaliquidity معفَون |
| إجراءات Mempool لكل حساب | 50 معلّقًا | يستنزف مع تأكيد الإجراءات | — | — |
| اشتراكات WS لكل اتصال | 64 | — | — | الاتصالات المُدرجة في القائمة البيضاء معفاة |

- **لكل IP**: يغطي حركة القراءة المجهولة (`/info`، الاشتراك عبر WS)؛ **عناوين IP المُدرجة في القائمة البيضاء** (صانعو السوق أو البنية التحتية المُعيَّنون من قِبل المشغّل) تتجاوز هذا الحد.
- **دلو الحساب** ينطبق على عمليات الكتابة الموقَّعة عبر `/exchange`. الحسابات ضمن **مجموعة مشغّلي metaliquidity** (الموقّعون المُدرجون في القائمة البيضاء لاستراتيجيات الخزائن) **معفاة** من دلو الحساب.
- **WS**: بحد أقصى **64 اشتراكًا نشطًا لكل اتصال**؛ يُرفض الاشتراك الخامس والستون. الاتصالات المُدرجة في القائمة البيضاء معفاة من هذا الحد.

جميع الحدود تخضع للحوكمة. يمكن الاطلاع على لقطة ميزانية الحساب عبر قراءة [`user_rate_limit`](./rest/info.md) الأصلية:
...
## الوزن حسب نقطة النهاية {#weight-by-endpoint}
...
عميل يُرسل أمرًا واحدًا في الثانية ويستطلع `account_state` مرة في الثانية يُنفق `5 + 1 = 6 وزن/ث = 360 وزن/دقيقة` — وهو في نطاق الميزانية بكل ارتياح.

## QPS لكل حساب {#per-account-qps}
...
## حد مجموعة الانتظار (Mempool) {#mempool-cap}
...
## سلوك الانطلاقة القصوى {#burst-behaviour}
...
### بوت تدفق الأوامر {#order-flow-bot}
...
### مستهلك بيانات السوق {#market-data-consumer}

- اشترك في قنوات WS (`l2_book`, `trades`, `user_events`)؛ لا تستطلع.
- وزن `subscribe` هو 1، والرسائل داخل التدفق تكلف 0.
- عند إعادة الاتصال، تُعيد الاشتراك من لقطة جديدة (لا توجد رموز استئناف)؛ كل اشتراك يُنفق الوزن مجددًا على الاتصال الجديد، لذا حافظ على الاتصالات طويلة الأمد. التزم بحد **64 اشتراكًا** لكل اتصال.

### منفّذ تصفية عالي التردد {#high-frequency-liquidator}
...
## التسلسل — التعرض للتقييد والتعافي منه {#sequence--getting-throttled-and-recovering}
...
## قنوات الاستثناء {#override-channels}
...
- [التعامل مع التكرار (Idempotency)](../integration/idempotency.md) — كيفية إعادة المحاولة ضمن ميزانية معدل الطلبات
