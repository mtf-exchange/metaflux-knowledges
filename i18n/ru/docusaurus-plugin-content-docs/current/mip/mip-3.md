...
Любой разработчик может развернуть новый рынок бессрочных контрактов на MetaFlux, оплатив участие через аукцион газа в сети. Никаких разрешений от команды протокола, никаких ревизионных комитетов, никаких белых списков. Единственными барьерами являются аукционная цена и минимальный депозит. (Развёртывание рынка **спот** без разрешений — родственное предложение [MIP-1](./mip-1.md).)

## Зачем это нужно

Это ключевая возможность протокола. Централизованные биржи самостоятельно отбирают листинги; MetaFlux делает сам процесс листинга частью протокола. Разработчикам, которым нужен рынок для нишевого актива, не требуется ничьё разрешение — нужно выиграть аукцион и предоставить начальные параметры.

Это адаптация MetaFlux подхода к развёртыванию рынков без разрешений, впервые реализованного ведущими on-chain площадками бессрочных контрактов, с сохранением следующих соответствий и корректировок:

- Три отдельных потока аукциона газа (`perp_deploy_gas_auction`, `spot_pair_deploy_gas_auction`, `register_token_gas_auction`) — та же структура, что и в HL. Развёртывание perp — MIP-3; спотовые потоки относятся к [MIP-1](./mip-1.md).
- Параметры аукциона (затухание, окно возврата средств, интервал слота) — настраиваются через управление протоколом
- Начальный коэффициент поддерживающей маржи, максимальное кредитное плечо, лимит финансирования — передаются вместе с заявкой на развёртывание в рамках диапазонов, установленных управлением протоколом
...
Развёртывание рынка бессрочных контрактов осуществляется через действие `perpDeploy`, диспетчируемое подвариантом `PerpDeployKind`, охватывающим полный жизненный цикл рынка (8 подвариантов):
...
3. **`SetLeverage`** — установка максимального лимита кредитного плеча.
...
Получение слота развёртывания происходит через аукцион газа: разработчик вызывает **`submitGasAuctionBid { auction_kind, bid_amount, ... }`** в соответствующем потоке. Каждая ставка содержит:
...
## Эскроу ставок и возврат средств
...
- Лимит финансирования ≤ `max_funding_per_hour` (по умолчанию 0.5%)
- Источник оракула из утверждённого списка

Ставки с параметрами, выходящими за допустимые границы, отклоняются при подаче.
...
- **Затухание** — темп снижения минимальной ставки при незанятом слоте (задаётся управлением протоколом, по умолчанию линейное снижение за 24 ч)
...
Все три параметра изменяются через управление протоколом с помощью действия `SetGlobal` (глобальные параметры управления разработчиков MIP-3: `SetGasAuctionDuration`, `SetMinDeployStake`, `SetGasAuctionMinBid`, `SetDeployerFeeCap`, `SetPerMarketLimits`, `SetEnableMip3`).
...
Разработчики, как правило, формируют глубину рынка, совмещая развёртывание по MIP-3 с источником ликвидности на том же рынке — [Металиквидность MIP-2](./mip-2.md), внешним маркет-мейкером, привлечённым скидками на комиссии разработчика, или пользовательским хранилищем.

## MIP-4

См. [MIP-4 — агрегатор / интернализатор ликвидности бессрочных контрактов](mip-4.md) — агрегатор MetaFlux, дополняющий развёртывание без разрешений.
