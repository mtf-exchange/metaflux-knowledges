...
:::info
**Имена каналов в snake_case (нативный формат MTF).** Эндпоинт `/ws` ноды использует нативный формат MTF, поэтому имена каналов передаются в snake_case: `l2_book`, `bbo`, `trades`, `active_asset_ctx`, `fills`, `candles`, `user_events`. Клиенты, желающие использовать имена каналов в camelCase в стиле HL (`l2Book`, `userEvents`, `userFills`, `candle`, …), подключаются к эндпоинту **`/hl/ws`** шлюза (режим совместимости с HL), который прозрачно транслирует запросы в нативный snake_case. Согласно маршрутизации единого шлюза: `<net>-gateway.mtf.exchange/ws` = нативный snake_case, `/hl/ws` = camelCase в стиле HL.
:::
...
Нативный WS MTF (каналы в snake_case) доступен по умолчанию на `/ws` шлюза; WS в режиме совместимости с HL (каналы в camelCase) — на `/hl/ws`. Шлюз терминирует TLS (`wss://`). При самостоятельном запуске ноды нативный WS доступен без шифрования по адресу `ws://localhost:8080/ws` — протокол кадров идентичен в обоих случаях.
...
- Флаг снимка в конверте **зависит от диалекта**, как и имена каналов в поле `type`: на нативном маунте `/ws` используется snake_case `is_snapshot`; на маунте `/hl/ws` в режиме совместимости с HL то же поле называется camelCase `isSnapshot`.
