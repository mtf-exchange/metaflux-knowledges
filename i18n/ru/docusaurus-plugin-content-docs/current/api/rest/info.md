...
Статические сведения о ноде и версия протокола. Параметры отсутствуют.
...
Ставки комиссий — десятичные **базисные пункты** в виде строк (`"2.0"` = 2 б.п. = 0,02%). `burn_ratio` — десятичная дробь (`"0.30"` = 30% комиссий сжигается). См. [комиссии](../../concepts/fees.md).

### `open_orders`

Активные отложенные ордера аккаунта по всем книгам бессрочных контрактов.
...
Аккаунт идентифицируется либо по `account_id` (u64), либо по `address` (0x hex). Если в запросе
указан `account_id`, он возвращается обратно в `data.account_id`.
...
| `account_id` | uint64 | Возвращается только если в запросе был указан `account_id` |
...
История исполнения ордеров аккаунта, обслуживаемая напрямую из зафиксированного состояния ноды (ограниченное кольцо исполнений на аккаунт, встроенное в AppHash — внешний индексер не требуется).
...
|-----|------|----------|-------------|
| `account_id` | uint64 | одно из `account_id` / `address` | Внутренний id аккаунта |
| `address` | hex address | одно из `account_id` / `address` | Адрес аккаунта |
...
| `address` | hex address | Разрешённый адрес аккаунта |
| `account_id` | uint64 | Возвращается только если в запросе был указан `account_id` |
| `fills[*].coin` | uint32 | Id актива / рынка, на котором произошло исполнение |
...
Зафиксированное подмножество источников оракула для указанного рынка. Рынок задаётся через `asset_id`
(u32) **или** `coin` (символ).
...
Если оба параметра отсутствуют — `400`; неизвестный рынок — `404 {"error":"market not found"}`.
...
| Поле | Тип | Описание |
|-------|------|-------------|
| `asset_id` | uint32 | Эхо-ответ / разрешённый идентификатор актива |
...
Объект `params` содержит полный набор управляемых параметров, которыми может оперировать машина голосования (распределение комиссий, настройки стейкинга, лимиты MIP-3, риск-кэпы, флаги спот / EVM / моста и т.д.); каждое значение является живым зафиксированным значением.
...
## Расширенные типы запросов (RFQ / FBA / портфельная маржа)
...
Запросы котировок (RFQ), в которых участвует аккаунт — разделённые на те, которые он инициировал, и те, по которым он выставлял котировки. См. [концепцию RFQ](../../concepts/rfq.md).
...
Аккаунт идентифицируется либо через `account_id` (u64), либо через `address` (0x hex); если в запросе передан `account_id`, он возвращается обратно в `data.account_id`. Отсутствие обоих полей → `400`; некорректный формат `address` → `400 {"error":"invalid hex"}`.
...
Отсутствие `market_id` → `400`. **404 не возвращается** для незарегистрированного рынка: FBA подключается на уровне отдельных рынков, поэтому рынок без пула возвращает **200** с нулевыми полями (`enabled:false`, `period_ms:0`, пустой `orders`, `indicative:null`).
...
Статус подключения к портфельной марже и последние вычисленные показатели сценариев для аккаунта. См. [Портфельная маржа](../../concepts/portfolio-margin.md).
...
Аккаунт идентифицируется через `account_id` (u64) или `address` (0x hex); отсутствие обоих → `400`. Аккаунт без подключения к портфельной марже возвращает **200** с `enrolled:false` и нулевыми показателями.
...
## Типы запросов к снимку состояния узла
...
## Общие типы запросов к снимку состояния узла

Запросы к снимку состояния узла, не привязанные к конкретному торговому продукту, — статус биржи, вспомогательные данные для интерфейса / открытых ордеров, ликвидация, лимиты частоты запросов, хранилища, валидаторы, мультиподпись и агрегированный `web_data2`.
...
Составной снимок «всего для фронтенда» для адреса. Обязательный параметр: `address` (hex, 0x). Составлен из других ридеров, поэтому форматы никогда не расходятся.
...
| Поле | Тип | Описание |
|-------|------|-------------|
| `clearinghouse.account_value` | decimal string | Стоимость кросс-аккаунта |
| `clearinghouse.margin_used` | decimal string | Σ использованной маржи по активам |
| `clearinghouse.positions` | object[] | Открытые позиции по каждому активу |
| `spot_balances` | object[] | Повторно использует `spot_clearinghouse_state.balances` |
| `open_orders` | object[] | Повторно использует `frontend_open_orders.orders` |
| `vault_equities` | object[] | Повторно использует `user_vault_equities.equities` |
| `exchange_status` | object | Повторно использует `exchange_status.data` |
...
| 400 | `{"error":"missing field: address"}` / `{"error":"missing field market_id"}` | Пропущен обязательный аргумент для данного типа (регистр зависит от ридера) |
| 400 | `{"error":"invalid hex"}` | Некорректный формат аргумента-адреса |
| 404 | `{"error":"market not found"}` | Неизвестный идентификатор актива или название монеты (только `market_info`) |
...
**Q: Зачем `market_info` принимает и `asset_id`, и `coin`?**
A: `asset_id` — канонический идентификатор; `coin` — удобный вариант для человека. Оба разрешаются в одну и ту же запись.
