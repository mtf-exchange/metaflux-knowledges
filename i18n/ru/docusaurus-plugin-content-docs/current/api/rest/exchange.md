...
|------|-----------|
| `POST /exchange` (шлюз по умолчанию) | **MTF-native** (этот документ) |
| `POST /hl/exchange` (шлюз, под `/hl`) | **HL-compat** — см. [hl-compat.md](./hl-compat.md) |

MTF-native — путь шлюза по умолчанию; HL-compat вынесен под пространство имён `/hl/*`.
При самостоятельном запуске узла тот же нативный `/exchange` доступен непосредственно на
`http://localhost:8080`.
...
:::warning
**`px` / `size` — беззнаковые `u64` с фиксированной точкой на нативном проводе**, передаются как JSON-числа (узел декодирует их как `u64`, затем расширяет внутри). Это отличается от пути HL-compat (десятичные строки). Адреса — `0x`-hex (40 символов);
`cloid` — `0x` + 32 hex-символа (16 байт).
:::

### Размещение и жизненный цикл ордера
...
| [`set_metaliquidity_whitelist`](#set_metaliquidity_whitelist) | Голосование за белый список MLP | ключ валидатора |
| [`register_metaliquidity_operator`](#register_metaliquidity_operator) | Зарегистрировать / отозвать оператора стратегии | лидер хранилища |
...
### Недоступно на публичном пути `/exchange`

Эти имена действий встречались в ранних редакциях (и некоторые — в HL-compat-интерфейсе),
...
| `AppHashVote` | — | Действие валидатора/системы; проходит через путь консенсуса, никогда через `/exchange` |
...
| (PM lifecycle) | `pm_enroll` / `pm_unenroll` / `pm_rebalance` | Распознанная, но не сопоставленная заготовка → `unsupported action` |
...
## Действия с бессрочными ордерами
...
#### Триггерные ордера (`stop_loss` / `take_profit`)
...
## Действия для спот-маржи и Earn
...
**Ответ.** Возвращает конверт допуска [`202 Accepted`](#202-accepted--non-order-admission) (не синхронный `oid`). Подтвердите зачисленный залог через [`/info` `spot_margin_state`](./info.md#spot_margin_state). Смотрите [спот-маржу](../../products/spot-margin.md).
...
**Ответ.** Возвращает конверт допуска [`202 Accepted`](#202-accepted--non-order-admission). Подтвердите через [`/info` `spot_margin_state`](./info.md#spot_margin_state).
...
**Ответ.** Возвращает конверт допуска [`202 Accepted`](#202-accepted--non-order-admission) (не синхронный `oid` — исполнение внутреннего IOC является зафиксированным эффектом). Наблюдайте за результирующими `borrowed` / `base_held` через [`/info` `spot_margin_state`](./info.md#spot_margin_state); `total_borrowed` пула Earn изменяется через [`earn_state`](./info.md#earn_state). Смотрите [спот-маржу](../../products/spot-margin.md).
...
**Ответ.** Возвращает конверт допуска [`202 Accepted`](#202-accepted--non-order-admission). Подтвердите полное или частичное закрытие и погашенную сумму через [`/info` `spot_margin_state`](./info.md#spot_margin_state) (удалённый счёт больше не отображается); эффекты на стороне поставщика видны через [`earn_state`](./info.md#earn_state).
...
**Ответ.** Возвращает конверт допуска [`202 Accepted`](#202-accepted--non-order-admission). Подтвердите выпущенные доли / вашу долю участия через [`/info` `earn_state`](./info.md#earn_state) (передайте `user` для включения `user_shares` / `user_value`). Смотрите [Earn](../../concepts/earn.md).
...
**Ответ.** Возвращает конверт допуска [`202 Accepted`](#202-accepted--non-order-admission); количество сожжённых долей может быть **меньше запрошенного**, если выплата была ограничена свободными средствами. Подтвердите оставшуюся долю участия и итоги пула через [`/info` `earn_state`](./info.md#earn_state). Смотрите [Earn](../../concepts/earn.md).

---

## Действия для маржи и управления рисками по бессрочным контрактам
...
## Действия для аккаунта, стейкинга, хранилищ и моста
...
Голосование валидаторов по управлению MIP-2: включение или исключение адреса из белого списка MLP. **Авторизация валидатором** — восстановленный подписант должен быть валидатором; изменение вступает в силу после достижения кворума по доле стейка валидаторов.
...
| Поле | Тип | Описание |
|-------|------|-------------|
| `address` | hex address | Адрес MLP, статус членства которого изменяется |
| `allowed` | bool | `true` = добавить в белый список; `false` = удалить |

---

### `register_metaliquidity_operator`

Действие лидера хранилища MIP-2: регистрация или отзыв внесетевого оператора стратегии в качестве авторизованного агента хранилища Metaliquidity. **Авторизация лидером хранилища** при отправке; оператор должен находиться в белом списке MLP.
...
| Поле | Тип | Описание |
|-------|------|-------------|
| `vault_id` | uint64 | Идентификатор целевого хранилища Metaliquidity |
| `operator` | hex address | Ключ внесетевой стратегии (должен быть в белом списке MLP) |
| `allowed` | bool | `true` = зарегистрировать как авторизованного агента; `false` = отозвать |
| `expires_at_ms` | uint64 \| null | Опциональный срок действия авторизации; `null` = бессрочно |
...
### Действия без проксирования через мост
...
| `AppHashVote` | — | Действие валидатора/системы; только путь консенсуса | — |
...
| (жизненный цикл PM) | `pm_enroll` / `pm_unenroll` / `pm_rebalance` | Распознанная, но не подключённая заглушка → `unsupported action` | [`user_portfolio_margin`](#user_portfolio_margin) для enroll/unenroll |
...
### `200 OK` — путь ордера (синхронный oid)
...
### `202 Accepted` — подтверждение приёма (не-ордер)
...
### `401 Unauthorized` — ошибка подписи / авторизации
...
### `503 Service Unavailable` — мемпул переполнен
...
## Диаграмма последовательности — размещение ордера и его появление в стакане
...
- [HL-совместимый `/exchange`](./hl-compat.md) — альтернативный формат для клиентов HL
