...
- **2xx** — успех. Обратите внимание: HL-совместимые эндпоинты возвращают `200 OK` даже при ошибках на уровне приложения и сигнализируют о них в теле ответа (`{"status":"err"}`). MTF-нативные эндпоинты используют корректные статус-коды.
...
HL-совместимые эндпоинты (`/info`, `/exchange` на шлюзе) оборачивают всё в:

```json
{ "status": "ok"|"err", "response": ... }
```

При `status: "err"` поле `response` содержит строку с описанием ошибки на уровне приложения при HTTP 200. Ошибки транспортного уровня (некорректный JSON, неверный метод) по-прежнему возвращаются с кодом 4xx.
...
Для запросов `/info`: MTF-нативные эндпоинты возвращают `404`; HL-совместимые возвращают `200` с телом `{"status":"err","response":"<msg>"}` (соглашение HL).
