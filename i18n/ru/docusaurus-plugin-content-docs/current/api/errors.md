...
## Структура тела ответа {#body-shape}
...
### 401 — не авторизован (ошибка подписи) {#401--unauthorized-signature-failed}
...
| `market not found` | Символ `coin` отсутствует в реестре |
...
### 405 — метод не разрешён {#405--method-not-allowed}

| `error` | Когда возникает |
|---------|----------------|
| (тело отсутствует) | Использован `GET` вместо `POST` (или наоборот) |

### 422 — необрабатываемый объект {#422--unprocessable-entity}
...
### 429 — превышение лимита запросов {#429--rate-limited}
...
### 503 — сервис недоступен {#503--service-unavailable}
...
### Ошибки при фиксации (не HTTP, в потоке событий) {#commit-time-errors-not-http-in-event-stream}

Часть сбоев происходит уже после `202 Accepted`, поскольку они выявляются только в контексте выполнения блока. Такие ошибки появляются в WS-канале `order_updates` / `user_events` в виде `{"error":"<reason>", "action_hash":"0x..."}`.
