...
Держите MTF, делегируйте их валидатору и получайте вознаграждения за стейкинг. Постоянный источник — комиссионный доход протокола: комиссии финансируют валидаторов — **долю валидаторов в 20%** от [выкупа за счёт комиссий](./fees.md) — а валидаторы финансируют стейкеров, передавая эту долю вниз за вычетом комиссии валидатора. На раннем этапе она дополняется конечным бюджетом первоначального стимулирования из казначейства (никогда не за счёт новой эмиссии). Стейк остаётся ликвидным вплоть до истечения `lock_period`; вывод из стейкинга занимает `7 дней` для полного разблокирования. Слэшинг применяется к нарушающим правила валидаторам; делегаторы несут частичную ответственность по слэшингу.
...
| **Протокол** | Распределяет вознаграждения за каждый блок пропорционально доле в стейке: доля валидаторов в комиссионном доходе плюс бюджет первоначального стимулирования из казначейства. |
...
|--------|---------|-------|
| Комиссионный доход — доля валидаторов в выкупе (комиссии → валидаторы → стейкеры) | За эпоху | `validator_share_inflow × stake_share × (1 - commission)` |
| Вознаграждения первоначального стимулирования (финансируются казначейством, ранняя фаза) | За каждый блок | `reward_per_block × stake_share × (1 - validator_commission)` |

Комиссионный доход — постоянный источник: согласно [маховику комиссий](./fees.md), выкупленные MTF распределяются как **70% сжигание / 20% валидаторы / 10% казначейство**, а доля валидаторов в 20% передаётся стейкерам за вычетом комиссии.
`reward_per_block`: задаётся управлением, выделяется из казначейского пула первоначального стимулирования — **не новая эмиссия**; текущее значение доступно через запрос `staking_state`.
`validator_commission`: устанавливается каждым валидатором, ограничена `20%` по решению управления.

Вознаграждения рассчитываются в MTF (вознаграждения первоначального стимулирования) и USDC (комиссионный доход) — при получении выплачиваются оба. В `staking_state` отображается ожидаемая сумма в каждой валюте.
...
Этот эффект вознаграждения при открытии блока финансируется из казначейского бюджета первоначального стимулирования (см. [токеномику](./tokenomics.md)) — вознаграждения за стейкинг никогда не полагаются на новую эмиссию. По мере роста комиссионного дохода доля валидаторов в 20% от [выкупа за счёт комиссий](./fees.md) становится доминирующим источником вознаграждений.
...
- [`POST /info staking_apr`](../api/rest/info.md#staking_apr) — эффективный APR вознаграждений первоначального стимулирования и связанные входные параметры
...
**В: Откуда берутся вознаграждения за стейкинг?**
О: Постоянный источник — комиссионный доход: валидаторы получают **долю валидаторов в 20%** от [выкупа за счёт комиссий](./fees.md) (70% сжигание / 20% валидаторы / 10% казначейство) и распределяют её среди своих стейкеров за вычетом комиссии. На раннем этапе её дополняет конечный бюджет первоначального стимулирования из казначейства. Протокол **никогда не выпускает новые MTF для вознаграждений** — единственный рычаг предложения это ежегодная переиндексация к численности населения ([токеномика](./tokenomics.md)).
