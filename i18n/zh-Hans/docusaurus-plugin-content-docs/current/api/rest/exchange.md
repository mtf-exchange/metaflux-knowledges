...
| [`set_metaliquidity_whitelist`](#set_metaliquidity_whitelist) | MLP 白名单投票 | validator key |
| [`register_metaliquidity_operator`](#register_metaliquidity_operator) | 注册 / 撤销策略运营者 | vault leader |
...
| `BorrowLend` | — | 未桥接 |
| `AppHashVote` | — | 验证者/系统操作；经共识路径处理，绝不走 `/exchange` |
...
| （PM 生命周期） | `pm_enroll` / `pm_unenroll` / `pm_rebalance` | 已识别但未映射存根 → `unsupported action` |
...
MIP-2 验证者治理投票：设置某地址在 MLP 白名单中的成员资格。**须经验证者授权** — 恢复的签名者必须为验证者；变更在达到验证者质押权重法定人数后生效。
...
| 字段 | 类型 | 说明 |
|-------|------|-------------|
| `address` | 十六进制地址 | 被设置成员资格的 MLP 地址 |
| `allowed` | bool | `true` = 加入白名单；`false` = 移除 |

---

### `register_metaliquidity_operator`

MIP-2 金库领导者操作：将链下策略操作员注册或撤销为 Metaliquidity 金库的授权代理人。**在提交时须经金库领导者授权**；操作员必须在 MLP 白名单中。
...
| `operator` | 十六进制地址 | 链下策略密钥（必须在 MLP 白名单中） |
| `allowed` | bool | `true` = 注册为授权代理人；`false` = 撤销 |
| `expires_at_ms` | uint64 \| null | 可选的授权到期时间；`null` = 永不过期 |
...
| （PM 生命周期） | `pm_enroll` / `pm_unenroll` / `pm_rebalance` | 已识别但未映射的存根 → `unsupported action` | 入组/退组使用 [`user_portfolio_margin`](#user_portfolio_margin) |
