...
- [从 HL 迁移](./migrating-from-hl.md) —— 将 Hyperliquid 机器人切换到 MTF 原生 API
...
网关（`https://api.<net>.mtf.exchange`）是唯一的公共入口。

| 路径 | 服务类型 | 用途 |
|------|--------|---------|
| `POST /info` · `POST /exchange` · `GET /ws` | MTF 原生 | 原生 snake_case 接口 |
